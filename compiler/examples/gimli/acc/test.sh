#!/bin/sh
# Functional test for the ACC gimli implementations (scalar + vectorized).
#
# For each: compiles the .jazz, assembles and links it with a small driver
# against the real ACC toolchain, runs it on the standalone simulator, and
...
"$JC" -arch acc "$HERE/gimli.jazz" -o "$BUILD/gimli.s"
...
"$JC" -arch acc "$HERE/gimliv.jazz" -o "$BUILD/gimliv.s"
...
echo
echo "All ACC gimli tests passed."
