...
- **`gimli.jazz`** -- scalar, using only the base (RISC-V) instruction set of
  the ISA (no wide `BN.*`). ACC's base core is a 32-bit RISC-V (RV32I-like)
  ...
  `add`/`xor`/`and`/`or`/shift. ACC has no rotate, so the column rotations are
...
```sh
../../../jasminc -arch acc gimli.jazz  -o gimli.s
../../../jasminc -arch acc gimliv.jazz -o gimliv.s
```

Or just check they assemble with the real toolchain:

```sh
../../../scripts/check-acc gimli.jazz
../../../scripts/check-acc gimliv.jazz
```

## Functional test

`test.sh` compiles each implementation, links it with a small driver, runs it
on the ACC standalone simulator, and compares the result of one
