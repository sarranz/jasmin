...
module Riscv_core = struct
  ...
  let asm_e = Riscv_extra.riscv_extra CInfo.instance atoI
  let aparams = Riscv_params.riscv_params CInfo.instance atoI
  let known_implicits = []
...
module Riscv (Lowering_params : Riscv_input) : Arch_full.Core_arch
  ...
  let not_saved_stack = (Riscv_params.riscv_liparams CInfo.instance atoI).lip_not_saved_stack
