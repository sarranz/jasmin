...
(* Strips up to two trailing "_wNN" suffixes, returning the indices in the
   order they appear in the source text (leftmost first). BN_MOVR needs
   two (destination, source); every other wide-register intrinsic needs at
   most one. *)
let get_wreg s =
  match get_wreg_once s with
  | None -> ((None, None), s)
  | Some (last, s') ->
    match get_wreg_once s' with
    | Some (first, s'') -> ((Some first, Some last), s'')
    | None -> ((None, Some last), s')
...
let tt_prim err ps s sa =
  let name, ofg, owb, ows, owr =
    ...
    | Some (Sopn.PVp ws) -> (s, None, None, Some ws, (None, None))
  ...
  | Sopn.PrimACC pr -> begin
      let pv =
        ...
        | Some ws, None,    None,    (None, None)   -> Sopn.PrimACCws ws
        | None,    _,       Some wb, (None, None)   -> Sopn.PrimACCwb (ofg, wb)
        | None,    Some fg, None,    (None, None)   -> Sopn.PrimACCfg fg
        | None,    None,    None,    (None, Some j) -> Sopn.PrimACCwreg (Conv.nat_of_int j)
        | None,    None,    None,    (Some i, Some j) ->
            Sopn.PrimACCwreg2 (Conv.nat_of_int i, Conv.nat_of_int j)
        | None,    None,    None,    (None, None)   -> Sopn.PrimACCnone
