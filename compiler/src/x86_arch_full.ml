...
let atoI decl =
  ...
  match Arch_extra.MkAToIdent.mk FInfo.instance decl mk_var with
  | Utils0.Error e ->
...
module X86_core = struct
  ...
  let asm_e = X86_extra.x86_extra FInfo.instance atoI
  let aparams = X86_params.x86_params FInfo.instance atoI

  let not_saved_stack = (X86_params.x86_liparams FInfo.instance atoI).lip_not_saved_stack
