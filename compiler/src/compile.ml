...
let do_spill_unspill asmop ?(debug = false) cp =
  ...
  match Lower_spill.spill_uprog FInfo.instance asmop Compiler.default_LoopCounter Conv.fresh_var_ident Conv.spill_to_mmx p with
...
let do_wint_int
  ...
  let cp = Wint_int.wi2i_prog FInfo.instance Arch.asmOp Arch.pointer_data Arch.msf_size get_info cp in
...
let compile (type reg regx xreg rflag cond asm_op extra_op)
  ...
  let pp_sr sr =
    ...
    pp_vbox FInfo.instance [
      pp_nobox FInfo.instance [
        PPEstring "{ region = ";
  ...
  Compiler.compile_prog_to_asm FInfo.instance Arch.asm_e Arch.call_conv Arch.aparams cparams
    export_functions
    (Expr.to_uprog FInfo.instance Arch.asmOp cprog)
