...
let to_array ty len t =
  ...
  ws, Array.init n (arr_get len Warray_.AAscale ws t)

let to_array8 ty len t =
  let ws, n = array_kind ty in
  Array.init (arr_size ws n) (arr_get len Warray_.AAdirect U8 t)
