...
let allocate_forced_registers return_addresses translate_var nv (vars: int Hv.t) (cnf: conflicts)
  ...
  | (StackByReg (ra,_) | ByReg (ra, _)), Exclusive (Arch_full.ByReg (Some r)) ->
...
let subroutine_ra_by_stack f =
  ...
  | Subroutine _ ->
      ...
      | StackDirect -> true
      | ByReg oreg ->
          (oreg <> None && has_call_or_syscall f.f_body)
          || f.f_annot.retaddr_kind = Some RAKstack
      | ByExtraReg -> false
       ...
       | Subroutine _ ->
           let ral =
             ...
             | Preferred ral -> begin
                 ...
                 | Some RAKstack -> StackDirect
                 | Some RAKregister -> ByReg None
                 | Some RAKextra_register -> ByExtraReg end
           in
           let get_r pre rk =
             V.mk
           ...
           | ByReg oreg ->
               ...
               in
               if need_stack then StackByReg (r, tmp) else ByReg (r, tmp)
           | ByExtraReg -> ByReg (get_r "ra" Extra, None)
    in
