  ...
  and analyse_instr_r (w, deps) ir =
    ...
    | Cfor (fi, c) -> (
        ...
        | FIrange (i, d, lo, hi) ->
            ...
            ( ( merge_writesets w w',
                after_join deps' (Sv.singleton (L.unloc i)) w' ),
              Cfor (FIrange (i, d, lo, hi), c) )
        | FIrepeat e ->
            let rec loop d =
              let (w', d'), c = analyse_stmt (empty_writeset, d) c in
              if is_included d' d then ((w', d), c)
            ...
            in
            let (w', deps'), c = loop deps in
            ( (merge_writesets w w', after_join deps' (vars_e e) w'),
              Cfor (FIrepeat e, c) ))
  ...
  let check_instr i =
    ...
    | Cfor (fi, _) -> (
        ...
        | FIrange (x, _, lo, hi) ->
            ...
            check ~loc deps s
        | FIrepeat e -> check ~kind:"Repeat loop count" ~loc deps (vars_e e))
    | _ -> ()
  in
