...
Definition acc_args : Type := seq lval * extended_op * seq pexpr.

Definition low_instr := lresult acc_args.
...
Definition low_cmd :=
  lresult (seq acc_args * seq lval * extended_op * seq pexpr).

Definition lc_sissue
  (pre : seq acc_args) (lvs : seq lval) (op : extended_op) (es : seq pexpr) :
  low_cmd :=
  issue (pre, lvs, op, es).
...
Definition with_pre (pre : seq acc_args) (li : low_instr) : low_cmd :=
  let%lr (lvs, op, es) := li in
...
Section LOWER_OPN.
  ...
  (* Match the arguments of a basic ACC operation and return the shift kind
  ...
  (* TODO_ACC: use the checker from the instruction description to fail early
     for unsafe programs. *)
  (* TODO_ACC: the other operations should be skipped, not give an error *)
  Definition lower_base_op
    (lvs : seq lval) (op : acc_op) (es : seq pexpr) : low_instr :=
    match op with
...
(* TODO_ACC define a helper for case analysis
  ...
  (* TODO_ACC Maybe we could extract from [instr_desc_op], but we need to fix
  ...
  Definition rv_Imn_of_op2
    ...
    | Osub (Op_w _) =>
        ...
        else Error (E.invalid_expr ii) (* TODO_ACC: better internal error *)
    | Oland _ => mk ANDI
    | Olor _ => mk ORI
    | Olxor _ => mk XORI
    | _ => skip
  ...
  Definition acc_Iop_of_op2
    (op : sop2) (fg : bn_flag_group) : lresult (seq lval * acc_op) :=
    match op with
  ...
  Definition lower_Papp2_large
    (ws : wsize) (op : sop2) (e0 e1 : pexpr) : low_instr :=
    let%lr (lvs, op) :=
      if isSome (is_wconst ws e1) then acc_Iop_of_op2 op FG0
      else acc_op_of_op2 op FG0
    in
