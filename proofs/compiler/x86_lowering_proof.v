...
Section PROOF.
  ...
  Definition mlower_prog :=
    (lowering.lower_prog (asmop := _asmop)
       (fun o w fv0 i => ok (lower_i o w fv0 i)) options warning fv).
  Notation lower_cmd := (conc_map (lower_i options warning fv)).
  Notation lower_prog :=
    (map_prog (fun fd =>
       with_body fd (conc_map (lower_i options warning fv) (f_body fd)))).

  Lemma lower_progE pp : mlower_prog pp = ok (lower_prog pp).
  Proof. by apply: (lower_prog_ext (gi := lower_i options warning fv)). Qed.
  ...
  Local
  Definition p' : prog (asmop := _asmop) := lower_prog p.

  Definition eq_exc_fresh s1 s2 := st_eq_ex fvars s1 s2.
  ...
  Lemma lower_condition_corr ii i e e' s1 cond :
    ...
    exists s2',
      [/\ esem p' ev (map (MkI ii) i) s1' = ok s2'
        , eq_exc_fresh s1 s2'
  ...
  Proof.
    ...
    all: have [s' [hsem heqf hseme]] := @sem_CMP s1' ii _ cf _ _ _ _ _ _ hws hseme0 hseme1 hw0 hw1.
    all: subst cf.
    all: eexists;
      split;
      first eassumption;
  ...
  Lemma opn_5flags_correct vi ii s a t o cf r xs ys m sz s' :
    ...
    ∃ s'',
    esem p' ev [seq MkI ii i | i <- opn_5flags fv m sz vi cf r t o a] s = ok s''
    ∧ eq_exc_fresh s' s''.
  Proof.
    ...
    + move: hopn => /opn_5flags_casesP [n ??]; subst a y.
      ...
      rewrite /sem_sopn /= /= /exec_sopn /= truncate_word_u /= -/(pwrepr64 _) write_var_eq_type //.
      rewrite /= /sem_sopn /= -/ℓ LetK.
      ...
      rewrite (eeq_exc_sem_pexprs dz e hz1) /=.
      move: hr.
      apply opn_no_immP.
  ...
  Lemma aux_eq_exc_trans P r r' :
    eq_exc_fresh r' r
    -> forall s,
        P s /\ eq_exc_fresh r s
        -> exists s', P s' /\ eq_exc_fresh r' s'.
  Proof.
    move=> Hr' s [Ps Hs].
    exists s.
    split; first exact Ps.
    exact: (eeq_excT Hr' Hs).
  Qed.
  ...
  Local Lemma Hassgn_esem ii l tag ty e s1 s2 s1' :
    let i := MkI ii (Cassgn l tag ty e) in
  ...
  Proof.
    ...
    + move=> b [tw hty'] [hle'] [sz'] [w] [hsz' ?]; subst v.
      ...
      * set wtmp := {| v_var := _ |}.
        ...
        assert (eq_exc_fresh s1' ℓ) as dℓ.
    ...
    + move=> _.
      ...
      exists s3; split => //.
      by rewrite map_cat esem_cat hs2'' /= /sem_assgn he' /= hty /= hw3.
    ...
    + move=> d u w s p0 p1 /= [[va [wa [hva hwa hdiv]]] _ hle1 hle2].
      ...
      have [s1'1 [hsem1 hget heq1]]: exists s1'1,
        [/\ esem_i p' ev (MkI ii i1) s1' = ok s1'1,
      ...
      + rewrite /i1 /va0; case: (u); eexists; split.
        ...
        split => //.
        by apply eq_exS, vmap_eq_except_set; apply multiplicand_in_fv.
      have [hwa1 [s3 [hsem heqe] {hdiv}]]:= hdiv _ heq1 Hdisjl Hdisje.
      exists s3;split.
      + move: hsem1 => /= -> /=.
        by case: d hsem; rewrite LetK /= /sem_sopn /= /get_gvar hget /= hwa1 /=; t_xrbindP => ? -> ? /= ->.
      apply: eeq_excT Hs2' heqe.
    (* LowerConcat *)
  ...
  Lemma lower_addcarry_correct ii si so si' sub sz xs t es x v :
    ...
    ∃ so',
      esem p' ev (map (MkI ii) (lower_addcarry fv sz sub xs t es)) si' = ok so' ∧
      eq_exc_fresh so so'.
  Proof.
    move=> hi dxs des hx hv ho.
    rewrite/lower_addcarry /=.
    set default := [:: Copn _ _ _ _ ].
    have hdefault : ∃ so', esem p' ev [seq MkI ii i | i <- default] si' = ok so' ∧ eq_exc_fresh so so'.
  ...
  Local Lemma Hopn_esem ii xs t o es s1 s2 s1':
    let i := MkI ii (Copn xs t o es) in
  ...
  Proof.
    ...
    have default : ∃ s2', esem p' ev [:: MkI ii (Copn xs t o es)] s1' = ok s2' ∧ eq_exc_fresh s2 s2'.
    + by exists s2'; split=> //; rewrite /= /sem_sopn Hx' /= LetK Hv.
    case: o Hv default => // -[] //;
    ...
    + move: Hv; rewrite /exec_sopn /sopn_sem ; t_xrbindP => _ hval <- y hy.
      ...
      have hdefault : ∃ s1'0,
          ...
          ∧ eq_exc_fresh s2 s1'0.
      + exists s2'; split=> //; apply: sem_seq1; apply: EmkI; apply: Eopn.
        by rewrite /sem_sopn /= /exec_sopn /sopn_sem /sopn_sem_ /= He1 He2 /= !truncate_word_le.
      rewrite /lower_mulu; case hsz: size_16_64 => //.
      have /andP [hsz16 hsz64] := hsz.
  ...
  Let Pi s (i:instr) s' :=
    disj_fvars (vars_I i) ->
    forall s1, eq_exc_fresh s s1 ->
      exists s1', sem p' ev s1 (lower_i options warning fv i) s1' /\ eq_exc_fresh s' s1'.
  ...
  Let Pc s (c:cmd) s' :=
    disj_fvars (vars_c c) ->
    forall s1, eq_exc_fresh s s1 ->
      exists s1', sem p' ev s1 (lower_cmd c) s1' /\ eq_exc_fresh s' s1'.

  Let Pfor (oi : option var_i) vs s c s' :=
    disj_fvars (Sv.union (sv_of_ovar_i oi) (vars_c c)) ->
    forall s1, eq_exc_fresh s s1 ->
      exists s1', sem_for p' ev oi vs s1 (lower_cmd c) s1' /\ eq_exc_fresh s' s1'.
  ...
  Proof.
    move=> s1 s2 x tag ty e v v' he htr hw ii hdisj s1' heqex.
    have /(_ s2) [|s2' [/esem_sem ??]]:= Hassgn_esem _ hdisj heqex.
    + by rewrite /sem_assgn he /= htr.
    by exists s2'.
  Qed.

  Local Lemma Hopn : sem_Ind_opn p Pi_r.
  Proof.
    move => s1 s2 t o xs es hsem ii hdisj s1' heqex.
    have [s2' [/esem_sem ??]]:= Hopn_esem hsem hdisj heqex.
    by exists s2'.
  Qed.
  ...
  Proof.
    ...
    + by rewrite /eq_exc_fresh /st_eq_ex /st_rel /=; case: hs1' => ?? ->.
  ...
  Proof.
    ...
    apply: (sem_app Hs2'1 (esem_sem Hs3'1)).
    by rewrite Hs3'3.
    exact: Hs4'1.
  ...
  Proof.
    ...
    exact: (sem_app Hs2'1 (esem_sem Hs3'1)).
    by rewrite Hs3'3.
  Qed.

  Local Lemma Hfor : sem_Ind_for p ev Pi_r Pfor.
  Proof.
    move=> s1 s2 fi c rn hfi _ hfor ii hdisj s1' hs1' /=.
    move: hdisj => /disj_fvars_vars_I_Cfor [hfvfi hfvuc].
    ...
    - have heq := write_fi_iterator fi.
      ...
      SvD.fsetdec.
    have [s2' [hs2'sem hs2'eq]] := hfor hpfor _ hs1'.
    exists s2'; split=> //.
    have hfi' : sem_fi true (p_globs p) s1' fi = ok rn.
    - clear hfor hfvuc hpfor hs2'sem.
      ...
      + move: hfi; rewrite /sem_fi /sem_pexpr_int /=.
        ...
        * apply: (disjoint_w _ hfvfi); rewrite /read_fi /read_fi_rec /= !read_eE; SvD.fsetdec.
        ...
        rewrite (eeq_exc_sem_pexpr hfvlo hs1' hlo) /= hzlo /=.
        by rewrite (eeq_exc_sem_pexpr hfvhi hs1' hhi) /= hzhi /=.
      + move: hfi; rewrite /sem_fi /sem_pexpr_int /=.
        t_xrbindP => z v hv hz <-.
        by rewrite /sem_fi /sem_pexpr_int /= (eeq_exc_sem_pexpr hfvfi hs1' hv) /= hz /=.
    apply: sem_seq1; apply: EmkI; apply: Efor; eauto.
  Qed.

  Local Lemma Hfor_nil : sem_Ind_for_nil Pfor.
  Proof. move=> s oi c _ s' Hs'; exists s'; split=> //; exact: EForDone. Qed.

  Local Lemma Hfor_cons : sem_Ind_for_cons p ev Pc Pfor.
  Proof.
    move=> s1 s1' s2 s3 oi w ws c hinit _ hc _ hfor hdisj s1'' hs1''.
    have hfvc : disj_fvars (vars_c c).
    - apply: (disjoint_w _ hdisj); SvD.fsetdec.
    have [s1_l hinit_l hs1_l] : exists2 s1_l,
        init_iteration true s1'' oi w = ok s1_l & eq_exc_fresh s1' s1_l.
    - clear hfor hc.
      case: oi hinit hdisj => [i | ] /= hinit hdisj.
      + have hfvi : disj_fvars (vars_lval i)
            by apply: (disjoint_w _ hdisj); rewrite /vars_lval /=; SvD.fsetdec.
        have [s1_l hw hs1_l] := eeq_exc_write_lval (gd := gd) hfvi hs1'' hinit.
        by exists s1_l.
      + case: hinit => <-.
        by exists s1''.
    have [s2'' [hs2''1 hs2''2]] := hc hfvc _ hs1_l.
    have [s3'' [hs3''1 hs3''2]] := hfor hdisj _ hs2''2.
    exists s3''; split=> //.
    apply: EForOne; eauto.
  Qed.
  ...
  Lemma lower_callP_total f scs mem scs' mem' va vr:
  ...
  Lemma lower_callP f scs mem scs' mem' va vr lp :
    mlower_prog p = ok lp ->
    sem_call p  ev scs mem f va scs' mem' vr ->
  ...
  Definition Pc_ (c : cmd) :=
    disj_fvars (vars_c c) ->
    wequiv_rec p p' ev ev eq_spec eq_exc_fresh c (lower_cmd c) eq_exc_fresh.
  ...
  Proof.
    ...
    eexists; first reflexivity.
    move=> s.
    set c' := conc_map (lower_i options warning fv) (f_body fd).
