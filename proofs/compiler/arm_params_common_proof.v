...
Proof.
  move=> hc h.
  have := [elaborate ARMFopn_coreP.movi_sem_fopn_args (s:=to_estate ls) hc h].
  by rewrite sem_fopn_equiv; apply: sem_fopn_args_eval_instr.
Qed.
...
Proof.
  ...
  have := [elaborate ARMFopn_coreP.gen_smart_opi_sem_fopn_args (is_small:= is_arith_small) (neutral:= Some 0%Z)
             (@ARMFopn_coreP.add_sem_fopn_args _) (@ARMFopn_coreP.addi_sem_fopn_args _)].
...
Proof.
  ...
  have := [elaborate ARMFopn_coreP.gen_smart_opi_sem_fopn_args (is_small:= is_arith_small) (neutral:= Some 0%Z)
              (@ARMFopn_coreP.sub_sem_fopn_args _) (@ARMFopn_coreP.subi_sem_fopn_args _)].
  move=> /(_ _ xi xi y imm s w) [] //.
