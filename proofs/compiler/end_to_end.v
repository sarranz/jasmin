...
Definition inv_mo : MoS -> MoT -> Prop.
Proof using. Admitted.

Definition inv_eq {X : Type} : X * MoS -> X * MoT -> Prop :=
  eqR (X := X) inv_mo.

Lemma inv_mo_mi : inv_mo mS xmT.
Proof using. Admitted.
...
Lemma inv_mo_full_pre fn xfd i ptrs ms mt :
...
have hpre := inv_mo_full_pre i ptrs hxfd hm.
apply: eutt_subrel; last first.
- apply: heq; first exact: hsafe.
  + exact: hdef.
  exact: hpre.
move=> fs xm hpost; by exists xfd, i, ptrs.
Qed.
...
Definition exec_rel'
  {X Y} (R : X -> Y -> Prop) (rx : execS X) (ry : execS Y) : Prop :=
  match rx, ry with
  | ESok x, ESok y => R x y
  | ESerror e1, ESerror e2 => e1 = e2
  | _, _ => False
  end.
...
- exact/eutt_to_Rnd/interp_exec_eutt/eutt_isem_res/hm.
