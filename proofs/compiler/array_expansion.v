...
Definition expand_return m ex x :=
  ...
  | Some (ws, len) =>
    match x with
    | Lnone v t => ok (nseq (Z.to_nat len) (Lnone v (aword ws)))
    | Lvar x =>
...
Fixpoint expand_i (m : t) (i : instr) : cexec instr :=
  ...
  | Cfor fi c =>
    Let _ :=
      ...
      let err := E.oreg_ierror i "reg array as a variable of a for loop" in
      add_iinfo ii (assert (Sv.subset (sv_of_ovar_i i) m.(svars)) err)
    in
