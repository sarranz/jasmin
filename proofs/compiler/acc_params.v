...
Module E.
  Definition pass := "ACC parameters"%string.
...
Section SAPARAMS.
  ...
  (* The operation computing [base + imm]: a register move when [imm] is
     ...
     extra op otherwise. The return type is pinned to [acc_extended_op]
     (rather than left to infer as the unfolded [extended_op_gen acc_op
  ...
  Definition add_imm_op (base : var_i) (imm : Z) : acc_extended_op * pexprs :=
 ...
 (* TODO_ACC: Is the LEA case correct? *)
  ...
  Definition immediate (x : var_i) (imm : Z) : instr_r :=
    Copn [:: Lvar x ] AT_none (Oacc (RV32 LI)) [:: cast_const imm ].
...
Section LIPARAMS.
  ...
  Definition vtmp : var := to_var X28. (* TODO_ACC: Is this a good choice? *)
  Definition vtmp2 : var := to_var X29. (* TODO_ACC: Is this a good choice? *)

  Let vtmpi := mk_var_i vtmp.
  Let vtmp2i := mk_var_i vtmp2.

  Definition fopn_args_of_opn_args (oa : ACCFopn_core.opn_args) : fopn_args :=
    let '(le, op, re) := oa in (le, Oacc op, re).

  (* TODO_ACC use the smart constructors? *)
  Definition allocate_stack_frame
    (rspi : var_i) (tmp : option var_i) (sz : Z) : seq fopn_args :=
    let c := [:: ACCFopn_core.subi rspi rspi sz ] in
    let c' :=
      let%opt aux := tmp in ACCFopn_core.smart_subi_tmp rspi aux sz
    in
    [seq fopn_args_of_opn_args x | x <- odflt c c' ].

  Definition free_stack_frame
    (rspi : var_i) (tmp : option var_i) (sz : Z) : seq fopn_args :=
    let c := [:: ACCFopn_core.addi rspi rspi sz ] in
    let c' :=
      let%opt aux := tmp in ACCFopn_core.smart_addi_tmp rspi aux sz
    in
    [seq fopn_args_of_opn_args x | x <- odflt c c' ].

  (* TODO_ACC move inside params_core *)
  Definition smart_addi x y imm :=
    let c := [:: ACCFopn_core.addi x y imm ] in
    let c' := ACCFopn_core.smart_addi x y imm in
    odflt c c'.

  (* TODO_ACC move inside params_core *)
  Definition smart_subi x y imm :=
    let c := [:: ACCFopn_core.subi x y imm ] in
    let c' := ACCFopn_core.smart_subi x y imm in
    odflt c c'.

  Definition set_up_sp_register
    (rspi : var_i) (sf_sz : Z) (al : wsize) (r : var_i) (tmp : var_i):
    seq fopn_args :=
    let c_copy := ACCFopn_core.smart_mov r rspi in
    let c_sub := smart_subi rspi r sf_sz in
    let c_align := [:: ACCFopn_core.align rspi rspi al ] in
    [seq fopn_args_of_opn_args a | a <- c_copy ++ c_sub ++ c_align ].

  Definition lmove (xd xs : var_i) : fopn_args :=
    fopn_args_of_opn_args (ACCFopn_core.mov xd xs).
  ...
  Definition lstore (ws : wsize) (xd : var_i) (ofs : Z) (xs : var_i) : fopn_args :=
    ...
    fopn_args_of_opn_args (ACCFopn_core.sw ws e xs).

  Definition lload (ws : wsize) (xd : var_i) (xs : var_i) (ofs : Z) :=
    let e := faddv Uptr xs (fconst ws ofs) in
    fopn_args_of_opn_args (ACCFopn_core.lw ws xd e).
...
Section AGPARAMS.
  ...
  (* TODO_ACC: Is this correct? It can be simplified *)
...
Definition acc_params : architecture_params :=
  {|
    ap_sap := saparams;
    ap_lip := liparams;
    ap_plp := true;
