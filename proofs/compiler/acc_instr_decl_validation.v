(* ACC instruction set -- validation.

   Sanity checks for the ACC instruction descriptions defined in
   [acc_instr_decl]: the primitive-string table ([VALIDATION_PRIM]), the
...
Section VALIDATION_PRIM.
  ...
  Let strings := Eval compute in map fst acc_prim_string.
...
Section VALIDATION_SEM.
  ...
  Notation test2 op w1 w2 r :=
    (is_ok
       (Let r' := id_semi (desc_acc_op op) w1 w2 in
        assert (wunsigned r' == wunsigned r) ErrSemUndef)).

  Notation test3 op w1 w2 w3 r :=
    (is_ok
       (Let r' := id_semi (desc_acc_op op) w1 w2 w3 in
        assert (wunsigned r' == wunsigned r) ErrSemUndef)).
  ...
  Notation test_so fg wb mf lf zf r x ix y iy acc sham mf' lf' zf' wrd_e acc_e :=
    (is_ok
       (Let res :=
          id_semi (desc_acc_op (BN_MULQACC_SO fg wb))
  ...
  Notation test_so_z fg wb mf lf zf r x ix y iy sham mf' lf' zf' wrd_e acc_e :=
    (is_ok
       (Let res :=
          id_semi (desc_acc_op (BN_MULQACC_SO_Z fg wb))
  ...
  Notation test_mq op x ix y iy acc sham r :=
    (is_ok
       (Let r' := id_semi (desc_acc_op op) x ix y iy acc sham in
        assert (wunsigned r' == wunsigned r) ErrSemUndef)).

  Notation test_mq_z op x ix y iy sham r :=
    (is_ok
       (Let r' := id_semi (desc_acc_op op) x ix y iy sham in
        assert (wunsigned r' == wunsigned r) ErrSemUndef)).
  ...
  Notation test_wo fg x ix y iy acc sham mf' lf' zf' wrd_e acc_e :=
    (is_ok
       (Let res :=
          id_semi (desc_acc_op (BN_MULQACC_WO fg)) x ix y iy acc sham
        in
        assert
          [&& res.1 == mf', res.2.1 == lf', res.2.2.1 == zf',
              wunsigned res.2.2.2.1 == wunsigned wrd_e
  ...
  Notation test_wo_z fg x ix y iy sham mf' lf' zf' wrd_e acc_e :=
    (is_ok
       (Let res :=
          id_semi (desc_acc_op (BN_MULQACC_WO_Z fg)) x ix y iy sham
        in
        assert
          [&& res.1 == mf', res.2.1 == lf', res.2.2.1 == zf',
              wunsigned res.2.2.2.1 == wunsigned wrd_e
  ...
  Goal is_ok (id_semi (desc_acc_op (BN_LID 5)) (wrepr U32 5) (wrepr U256 42)).
  Proof. by []. Qed.

  Goal is_ok (id_semi (desc_acc_op (BN_LID 0)) (wrepr U32 0) (wrepr U256 0)).
  Proof. by []. Qed.

  Goal is_ok (id_semi (desc_acc_op (BN_LID 31)) (wrepr U32 31) (wrepr U256 0xdeadbeef)).
  Proof. by []. Qed.

  (* Failure: first arg does not match the instruction's register index. *)
  Goal ~is_ok (id_semi (desc_acc_op (BN_LID 5)) (wrepr U32 6) (wrepr U256 42)).
  Proof. by []. Qed.

  Goal ~is_ok (id_semi (desc_acc_op (BN_LID 0)) (wrepr U32 1) (wrepr U256 0)).
  Proof. by []. Qed.

  Goal ~is_ok (id_semi (desc_acc_op (BN_LID 31)) (wrepr U32 0) (wrepr U256 0)).
  Proof. by []. Qed.
  ...
  Goal is_ok (id_semi (desc_acc_op (BN_SID 5)) (wrepr U256 42) (wrepr U32 5)).
  Proof. by []. Qed.

  Goal is_ok (id_semi (desc_acc_op (BN_SID 0)) (wrepr U256 0) (wrepr U32 0)).
  Proof. by []. Qed.

  Goal is_ok (id_semi (desc_acc_op (BN_SID 31)) (wrepr U256 0xdeadbeef) (wrepr U32 31)).
  Proof. by []. Qed.

  (* Failure: second arg does not match the instruction's register index. *)
  Goal ~is_ok (id_semi (desc_acc_op (BN_SID 5)) (wrepr U256 42) (wrepr U32 6)).
  Proof. by []. Qed.

  Goal ~is_ok (id_semi (desc_acc_op (BN_SID 0)) (wrepr U256 0) (wrepr U32 1)).
  Proof. by []. Qed.

  Goal ~is_ok (id_semi (desc_acc_op (BN_SID 31)) (wrepr U256 0) (wrepr U32 0)).
  Proof. by []. Qed.
