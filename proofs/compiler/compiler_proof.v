...
Definition wequiv_f_trans'
  ...
  {pT1 pT2 pT3 : progT}
  {E E0 : Type -> Type}
  {sem_F1 : sem_Fun (pT := pT1) E}
  {sem_F2 : sem_Fun (pT := pT2) E}
  {sem_F3 : sem_Fun (pT := pT3) E}
  {wE : with_Error E E0}
  {rE0 : EventRels E0}
  {ERT : EventRels_trans rE0}
...
#[local]
Instance HandlerContract_trans : EventRels_trans HandlerContract :=
  {|
...
Definition wequiv_f_trans_X_EU dc1 dc2 dc3 (p1 p2 p3 : prog) ev fn eS :=
  wequiv_f_trans'
    (sem_F1 := sem_fun_full (dc := dc1))
    (sem_F2 := sem_fun_full (dc := dc2))
    (sem_F3 := sem_fun_full (dc := dc3))
    (ERT := HandlerContract_trans)
    (p1 := p1) (p2 := p2) (p3 := p3)
...
Definition wequiv_f_trans_EE_EU dc1 dc2 dc3 p1 p2 p3 ev fn :=
  wequiv_f_trans_X_EU
...
(* wequiv eq eq -> wequiv eq uincl -> wequiv eq uincl *)
Definition wequiv_f_trans_EU_EU dc1 dc2 dc3 p1 p2 p3 ev fn :=
  wequiv_f_trans_X_EU
    (p1 := p1) (p2 := p2) (p3 := p3) (ev := ev)
    (dc1 := dc1) (dc2 := dc2) (dc3 := dc3)
    (eS := eq_uincl_spec)
...
Definition wequiv_f_trans_UU_EU dc1 dc2 dc3 p1 p2 p3 ev fn :=
  wequiv_f_trans_X_EU
    (p1 := p1) (p2 := p2) (p3 := p3) (ev := ev)
    (dc1 := dc1) (dc2 := dc2) (dc3 := dc3)
    (eS := uincl_spec)
...
Definition wequiv_f_trans_UU_UU dc1 dc2 dc3 (p1 p2 p3 : prog) ev fn :=
  wequiv_f_trans'
    (sem_F1 := sem_fun_full (dc := dc1))
    (sem_F2 := sem_fun_full (dc := dc2))
    (sem_F3 := sem_fun_full (dc := dc3))
...
Lemma it_inliningP to_keep (p p' : uprog) ev fn :
  inlining cparams to_keep p = ok p' ->
  wequiv_f
    (sem_F1 := sem_fun_full (dc := indirect_c))
    (sem_F2 := sem_fun_full (dc := indirect_c))
...
Lemma it_dead_code_callPu
  ...
  (forall (op : asm_op_t) (vx : value) (v : values),
    ...
    wequiv_f
      (sem_F1 := sem_fun_full (dc := dc))
      (sem_F2 := sem_fun_full (dc := dc))
...
Lemma hlop_it_lower_callP {p : prog} {ev : extra_val_t} {fn : funname} :
  ...
  wequiv_f
    (sem_F1 := sem_fun_full (dc := direct_c))
    (sem_F2 := sem_fun_full (dc := direct_c))
    p p' ev ev (rpreF (eS := eq_spec)) fn fn (rpostF (eS := eq_spec)).
Proof using. Admitted.
...
Proof.
move=> hp'.
apply: wequiv_f_trans_UU_UU; first exact: it_const_prop_callP.
...
t_xrbindP=> p0 hp0 hp'.
apply: wequiv_f_trans_UU_UU; last exact: hind hp0 hp'.
apply: wequiv_f_trans_UU_UU; first exact: it_postprocessP hpu.
...
apply: wequiv_f_trans_UU_EU; first exact: (it_alloc_call_uprogP _ _ ok_p').
...
apply: wequiv_f_trans_UU_EU; first exact: it_wi2w_progP ok_paw.
apply: wequiv_f_trans_UU_EU; first exact: (it_array_copy_fdP _ ok_pa0).
apply: wequiv_f_trans_EE_EU; first exact: it_add_init_callP.
apply: wequiv_f_trans_EE_EU; first exact: (it_alloc_callP _ ok_pb).
apply: wequiv_f_trans_EE_EU; first exact: it_inliningP ok_pa.
apply: wequiv_f_trans_UU_EU; first exact: it_unrollP ok_pc.
apply: wequiv_f_trans_EE_EU;
  first exact: (it_dead_calls_err_seqP ok_pd _ ok_fn).
apply: wequiv_f_trans_EU_EU; first exact: it_live_range_splittingP ok_pe.
apply: wequiv_f_trans_UU_EU; first exact: (it_remove_init_fdPu is_reg_array).
apply: wequiv_f_trans_EE_EU.
- apply: (wkequiv_io_weaken (P := rpreF (eS := mra_spec _) fn fn)) => //;
    last exact: (it_makeReferenceArguments_callP _ ok_pf).
  by move=> ???? [_ <-] [<-].
apply: wequiv_f_trans_UU_EU; first exact: it_indirect_to_direct.
apply: wequiv_f_trans_EE_EU; first exact: (it_expand_callP ok_pg ok_fn).
apply: wequiv_f_trans_EU_EU; first exact: it_live_range_splittingP ok_ph.
apply: wequiv_f_trans_EE_EU; first exact: RGP.it_remove_globP ok_pi.
apply: wequiv_f_trans_EE_EU; first exact: (it_load_constants_progP ok_plc).
apply: wequiv_f_trans_EE_EU; first exact: hlop_it_lower_callP ok_fvars.
apply: wequiv_f_trans_UU_EU; first exact: (it_pi_callP _ ok_pj).
apply: wequiv_f_trans_EE_EU.
