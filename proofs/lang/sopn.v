...
HB.instance Definition _ := hasDecEq.Build internal_reason internal_reason_eq_axiom.
...
HB.instance Definition _ := hasDecEq.Build internal_op internal_op_eq_axiom.
...
Fixpoint use_vars_semi_aux
  (tin tout : seq stype) : sem_prod tin (sem_tuple tout) :=
  match tin with
  | [::] => mk_ltuple default_value tout
  | _ :: tin => fun _ => use_vars_semi_aux tin tout
  end.

Definition use_vars_semi tin tout :=
  sem_prod_ok tin (use_vars_semi_aux tin tout).
...
Definition use_vars_instr
  ...
  let semi_aux := use_vars_semi_aux tin tout in
  {|
    ...
    i_safe := [::];
    i_safe_wf := refl_equal;
    i_semi_errty := fun _ => (@sem_prod_ok_error _ tin semi_aux ErrType);
    i_semi_safe  := fun _ => (@sem_prod_ok_safe _ tin semi_aux);
  |}.
