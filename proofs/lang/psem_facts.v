...
Lemma mem_equiv_for fi c : Pc c → Pr (Cfor fi c).
Proof.
  move=> h ii s1 s2 /=; t_xrbindP => rn _.
  elim: rn s1 => [ | j js ih] s1 /=.
  + by move=> [<-].
  t_xrbindP => sB sA hA hB hrec.
  have [vm heq _] := init_iteration_estateP hA.
  have hmem : emem s1 = emem sA by rewrite heq.
  rewrite hmem; etransitivity; [exact: (h _ _ hB) | exact: (ih _ hrec)].
Qed.
